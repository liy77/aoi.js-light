module.exports = async d => {
	...
	if (!inside.inside) throw new Error(`:x: Invalid usage in $getInviteInfo${inside}`)
    
    const [c, option] = inside.splits
    
    const invites = await d.message.guild.fetchInvites().catch(err => null) 
    
    if (!invites) throw new Error(`❌ Failed to fetch invites`)
    
    const invite = invites.find(e => e.url === c || e.code === c) 
    
    if (!invite) throw new Error(`❌ Invalid invite code in \`$getInviteInfo${inside}\``) 
