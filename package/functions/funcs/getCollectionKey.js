module.exports = async d => {
	...
	if (!inside.inside) throw new Error(`:x: Invalid usage in $getCollectionKey${inside}`)
    
    const [cname, key] = inside.splits 
    
    const collection = d.client.collections[cname.addBrackets()]
    
    if (!collection) throw new Error(`❌ Invalid collection name in \`$getCollectionKey${inside}\``) 
