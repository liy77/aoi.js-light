module.exports = async d => {
	...
	if (!inside.splits.length) throw new Error(`:x: Invalid usage in $moveUser${inside.total}`)

	const [ userID, channel = null, reason ] = inside.splits

	const user = await d.message.guild.members.fetch(userID).catch(err => {})

	if (!user) throw new Error(`:x: Invalid userID in \`$moveUser${inside.total}\``)

	const state = d.message.guild.voiceStates.cache.get(user.id)

	if (!state || !state.channel) throw new Error(`:x: User is not in any voice channel in \`$moveUser${inside.total}\``)

	try {
		await state.setChannel(channel && channel.length ? channel : null, reason)
	} catch {
		throw new Error(`:x: Failed to move user from a voice channel in \`$moveUser${inside.total}\``)
	}
