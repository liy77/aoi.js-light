...
module.exports = async d => {
	...
	if (!url) throw new Error(`:x: No url specified in \`$httpRequest${inside}\``)

	url = url.addBrackets()
	method = method.toUpperCase()

	if (!methods.includes(method)) throw new Error(`:x: Invalid method '${method}' in \`$httpRequest${inside}\``)
