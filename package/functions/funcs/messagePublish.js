module.exports = async d => {
    ...
    if (inside.inside) {
        ...
        if (!channel) throw new Error(`❌ Invalid channel ID in \`$messagePublish${inside}\``) 
        
        const msg = await channel.messages.fetch(messageID).catch(err => null) 
        
        if (!msg) throw new Error(`Invalid message ID in \`$messagePublish${inside}\``) 
        
        const m = await msg.crosspost().catch(err => null) 
        
        if (!m) throw new Error(`❌ Could not publish message `) 
