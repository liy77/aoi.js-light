module.exports = async d => {
    ...
    if (inside.inside) {
        ...
        if (!channel) return throw new Error(`❌ Invalid channel ID in \`$messagePublish${inside}\``) 
        
        const msg = await channel.messages.fetch(messageID).catch(err => null) 
        
        if (!msg) return throw new Error(`Invalid message ID in \`$messagePublish${inside}\``) 
        
        const m = await msg.crosspost().catch(err => null) 
        
        if (!m) return throw new Error(`❌ Could not publish message `) 
