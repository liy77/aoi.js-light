...
if (inside.splits.length > 3) return throw new Error(`❌ Too many fields in \`$random[${inside}]\``)
    if (isNaN(Number(n1)) || isNaN(Number(n2)) || Number(n1) >= Number(n2)) return throw new Error(`:x: Invalid number in \`$random[${inside}]\``)
