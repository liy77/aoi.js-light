...
module.exports = async d => {
    ...
    if (!member) throw new Error(`❌ Invalid user ID in \`$hasAnyRole${inside}\``)

    const roles = fields 

    if (roles.some(id => d.message.guild.roles.cache.get(id) === undefined)) throw new Error(`Invalid role ID given in \`$hasAnyRole${inside}\``)
