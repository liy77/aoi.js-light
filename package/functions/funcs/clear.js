module.exports = async (d) => {
  ...
  if (!channel) throw new Error(`:x: Invalid channel ID in \`$clear${inside}\``);
  ...
  while (input > 0) {
    ...
    if (!messages) throw new Error(":x: Failed to fetch messages");
    ...
    if (!deleted) throw new Error(":x: Failed to delete messages");
