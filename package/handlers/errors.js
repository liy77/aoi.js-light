...
const errorHandler = async (d, errorMessage, returnMsg = false, channel) => {
	...
	if (errorMessage.includes("{file:")) {
	    for (const after of errorMessage.split("{file:").slice(1)) {
	        ...
	        embed.attachFiles(new MessageAttachment(Buffer.from(text), name))
	...
	if (errorMessage.includes("{attachment:")) {
	    for (const after of errorMessage.split("{attachment:").slice(1)) {
	        const inside = after.split("}")[0];let[name,...url]=inside.split(":");name=name.addBrackets();url=url.join(":").addBrackets();const attachment=new MessageAttachment(url, name);embed.attachFiles(attachment);errorMessage=errorMessage.replace(`{attachment:${inside}}`, "")
	    }
	}
